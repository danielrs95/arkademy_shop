...
RSpec.describe '/admin/products', type: :request do
  ...
  describe 'as an admin' do
    before(:each) do
      admin = User.create(admin: true, email: 'email@admin.pl', password: 'daniel123')
      sign_in(admin)
    end
    ...
    describe 'PATCH /update' do
      context 'with valid parameters' do
        let(:new_attributes) { { name: 'Sneakers', price: 49.99, quantity: 4 } }
  ...
  describe 'as a user' do
    before(:each) do
      user = User.create(admin: false, email: 'email@admin.pl', password: 'daniel123')
      sign_in(user)
    end
